...
def xxz_data():
    ...
    xs = []
    ys = []
    for x, y in zip(data["xs"][107:189:2], data["ys"][107:189:2]):
        xs.append(x)
        ys.append(0)
    for x, y in zip(data["xs"][200:-12:2], data["ys"][200:-12:2]):
        xs.append(x)
        ys.append(1)
    xs0 = np.array(xs)
    ys0 = np.array(ys)
    ...
    xs_val = []
    ys_val = []
    for x, y in zip(data["xs"][106:190:2], data["ys"][106:190:2]):
        xs_val.append(x)
        ys_val.append(0)
    for x, y in zip(data["xs"][199:-11:2], data["ys"][199:-11:2]):
        xs_val.append(x)
        ys_val.append(1)
...
def mnist_data(num_train):
    ...
    y_train_1_9_binary = np.where(
        y_train_1_9 == 1, 0, 1
    )  # 0 if 1, 1 if 9 (i.e. if 9 then 1)
