...
# xxz phase
resultsqnny = qnn_y_flip(xs0,ys0,xs_val,ys_val,ave=5,dep=4,epo=200,bat=32,lr=0.03)
resultsqnnx = qnn_x_poison(xs0,ys0,xs_val,ys_val,ave=5,dep=4,epo=200,bat=32,lr=0.03)
resultsmlpy = mlp_y_flip(xs0,ys0,xs_val,ys_val,ave=5,hid=[64, 16],epo=400,bat=32,lr=0.01)
resultsmlpx = mlp_x_poison(xs0,ys0,xs_val,ys_val,ave=5,hid=[64, 16],epo=400,bat=32,lr=0.01)

resultsmlpy2 = mlp_y_flip(xs0,ys0,xs_val,ys_val,ave=5,hid=[32, 8],epo=400,bat=32,lr=0.01)
resultsmlpy3 = mlp_y_flip(xs0,ys0,xs_val,ys_val,ave=5,hid=[16, 4],epo=400,bat=32,lr=0.01)
resultsmlpy4 = mlp_y_flip(xs0,ys0,xs_val,ys_val,ave=5,hid=[8, 2],epo=400,bat=32,lr=0.01)
resultsmlpy5 = mlp_y_flip(xs0,ys0,xs_val,ys_val,ave=5,hid=[8, 4],epo=400,bat=32,lr=0.01)
np.save("xxz_phase.npy",[resultsqnny,resultsmlpy,resultsqnnx,resultsmlpx,resultsmlpy2,resultsmlpy3,resultsmlpy4,resultsmlpy5])
...
mode = 'ga'
resultga = mlp_unlearn(model,mode,xs_train,ys_train,xs_retain, ys_retain,
                       xs_forget_pollu,ys_forget,xs_val,ys_val, ave,epo,bat,lr,kl_w=0.,ce_w=1.,fo_w=0.2)
...
final_re = []
for fo_w in np.arange(0., 1.0, 0.1):
    results = mlp_unlearn(model,mode,xs_train,ys_train,xs_retain, ys_retain,xs_forget_pollu,ys_forget,xs_val,ys_val, ave,epo,bat,lr,kl_w=0.,ce_w=1.,fo_w=fo_w)
    val_accs=[results[i]["metrilist"] for i in range(len(results))]
    acc=[]
    for i in range(ave):
        for j in range(len(val_accs[i])):
